...
export function ChartPreview() {
  ...
  // Generate random data only on the client-side, once on mount
  useEffect(() => {
    const data: ChartDataPoint[] = Array.from({ length: 30 }).map((_, i) => {
      ...
      return {
        volHeight: Math.random() * 40,
        volY: 280 - Math.random() * 40,
        isGreen,
        wickHigh: 250 - high,
        wickLow: 250 - low,
        bodyY: 250 - Math.max(open, close),
        bodyHeight: Math.abs(open - close) || 2
      }
    })
    setChartData(data)
  }, [])
          ...
          >
            <Card className="p-6 max-w-5xl mx-auto">
              ...
              <div className="bg-muted/50 rounded-lg p-4 h-96 relative">
                {/* Grid Lines - These are static, safe to render always */}
                ...
                {chartData.length === 0 ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <div className="animate-pulse flex space-x-4">
                        <div className="h-40 w-full bg-muted-foreground/10 rounded"></div>
                      </div>
                    </div>
                ) : (
                    <svg viewBox="0 0 800 300" className="w-full h-full">
                      {chartData.map((d, i) => (
                          <g key={`group-${i}`}>
                            ...
                            <g>
                              <line
                                  x1={i * 26 + 15} y1={d.wickHigh}
                                  x2={i * 26 + 15} y2={d.wickLow}
                                  stroke={d.isGreen ? "oklch(0.60 0.18 145)" : "oklch(0.60 0.25 25)"}
                                  strokeWidth="1"
                              />
                              <rect
                                  x={i * 26 + 8}
                                  ...
                                  width="14"
                                  height={d.bodyHeight}
                                  fill={d.isGreen ? "oklch(0.60 0.18 145)" : "oklch(0.60 0.25 25)"}
                              />
                            </g>
                          </g>
                      ))}
